...
/* =========================================================
   SINGLE TRIONN PIECE
========================================================= */
function Piece({ position, rotation = [0, 0, 0], scale, orange = false }) {
  return (
    <RoundedBox
      args={[1, 1, 1]}
      radius={0.075}
...
export default function TrionnMark() {
  ...
  useFrame((state, delta) => {
    ...
    introProgress.current = THREE.MathUtils.clamp(time / introDuration, 0, 1);
    /*
      Smooth easing.
    */
    const introEase =
    ...
    */
    ...
    /* =====================================================
       ...
       assembled → exploded
    ===================================================== */
    const breakProgress = THREE.MathUtils.clamp(scrollProgress.current, 0, 1);
    /*
      Smooth easing for breaking.
    */
    const breakEase = breakProgress * breakProgress * (3 - 2 * breakProgress);
    /* =====================================================
       4. LOGO GROUP POSITION
       Mouse parallax.
    ===================================================== */
    const targetPositionX = 0.7 + pointer.x * 0.18;
    const targetPositionY = pointer.y * 0.12;
    ...
    */
    ...
    ===================================================== */
    const targetRotationX = pointer.y * 0.025;
    ...
    ===================================================== */
    const targetRotationZ = pointer.x * 0.045;
    ...
    if (shineLight.current) {
      const radius = 3.2;
      shineLight.current.position.x = Math.cos(rotationY.current) * radius;
      shineLight.current.position.z = Math.sin(rotationY.current) * radius;
      shineLight.current.position.y = 0.5 + Math.sin(time * 1.2) * 0.35;
    }
    /* =====================================================
       9. MOVE EVERY PIECE
       Intro:
    ...
    pieces.forEach((piece, index) => {
      const child = group.current.children[index + 1];
      /*
      ...
      */
      ...
      /* ===================================================
         INTRO PHASE
      =================================================== */
      if (introProgress.current < 1 && currentScroll < 20) {
        /*
      ...
      */
      child.position.x = THREE.MathUtils.lerp(child.position.x, x, 0.11);
      child.position.y = THREE.MathUtils.lerp(child.position.y, y, 0.11);
      child.position.z = THREE.MathUtils.lerp(child.position.z, z, 0.11);
    });
  });
  /* =========================================================
     RENDER
